...
import { getRemindersArrFilterByWeek, getRemindersObjByHourStart } from './getReminders';
import { showPopUpForm } from './renderPopUp';
...
const getHourCellsElements = (remindersPerDay) => {
  ...
  const arrDayCellsElems = arrOfHours.map((i) => `<div class="day-column__hour-cell" id="${i < 10 ? `0${i}` : i}:00">${getRemindersPerCell(remindersPerDay, i)}</div>`);
...
const onCalendarClick = (event) => {
  if (event.target.className == 'day-column__reminder-item') {
    ...
    const getObjectById = getStorage().find((item) => item.id === event.toElement.id);
